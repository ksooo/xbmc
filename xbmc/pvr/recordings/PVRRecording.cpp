...
{
  std::shared_ptr<CPVRClient> client = CPVRManager::Get().GetClient(m_iClientId);
...
std::shared_ptr<CPVRChannel> CPVRRecording::Channel(void) const
{
  if (m_iChannelUid != PVR_CHANNEL_INVALID_UID)
    return CPVRManager::Get().ChannelGroups()->GetByUniqueID(m_iChannelUid, m_iClientId);
...
{
  const std::vector<std::shared_ptr<CPVRTimerInfoTag>> recordingTimers = CPVRManager::Get().Timers()->GetActiveRecordings();
