...
{
  CLog::Log(LOGINFO, "Music database: create artist table");
  ...
  CLog::Log(LOGINFO, "Music database: create album table");
  ...
  CLog::Log(LOGINFO, "Music database: create audiobook table");
...
void CMusicDatabase::CreateAnalytics()
{
  ...
  CLog::Log(LOGINFO, "Music database: create triggers");
...
{
  CLog::Log(LOGINFO, "Music database: create song view");
  ...
  CLog::Log(LOGINFO, "Music database: create album view");
  ...
  CLog::Log(LOGINFO, "Music database: create artist view");
  ...
  CLog::Log(LOGINFO, "Music database: create albumartist view");
  ...
  CLog::Log(LOGINFO, "Music database: create songartist view");
...
void CMusicDatabase::CreateNativeDBFunctions()
{
  ...
  if (!StringUtils::EqualsNoCase(
          ...
          "mysql"))
    return;
  CLog::Log(LOGINFO, "Music database: create native MySQL/MariaDB functions");
...
bool CMusicDatabase::UpdateAlbum(CAlbum& album)
{
  ...
  if (album.bBoxedSet)
  {
    ...
    { // not a boxset already so make sure we have enough discs & they all have titles
      ...
      {
        CLog::Log(LOGINFO,
                  "Music database: Album with id [{}] does not meet the requirements for a boxset.",
                  album.idAlbum);
...
int CMusicDatabase::Cleanup(CGUIDialogProgress* progressDialog /*= nullptr*/)
{
  ...
  CLog::Log(LOGINFO, "Music database: Starting musicdatabase cleanup ..");
  ...
  CLog::Log(LOGINFO, "Music database: Cleaning musicdatabase done. Operation took {} s",
            duration.count());
...
{
  CLog::Log(LOGINFO, "Music database: updating tables");
  if (version < 34)
  {
                                       ...
                                       CGUIDialogProgress* progressDialog)
{
  ...
  try
  {
    ...
    CLog::Log(LOGINFO, "Music database: Create temporary HistSong table and insert {} records",
              total);
    ...
    */
    ...
    CLog::Log(LOGINFO, "Music database:  Importing song history {} of {} songs matched",
