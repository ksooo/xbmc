                                     ...
                                     const std::function<void(CPVRClient* client)>& function)
{
  ...
  {
    CLog::LogFunction(LOGDEBUG, strFunctionName, LOGPVR, "Ignoring callback from PVR client '{}'",
                      client->GetFriendlyName());
    return;
  }

  // Call.
  client->m_allAddonCallsFinished.Reset();
  client->m_iAddonCalls++;

  function(client);

  client->m_iAddonCalls--;
  if (client->m_iAddonCalls == 0)
    client->m_allAddonCallsFinished.Set();
}
                                             ...
                                             const PVR_RECORDING* recording)
{
  HandleAddonCallback(__func__, kodiInstance, [&](CPVRClient* client) {
    ...
    recordings->UpdateFromClient(transferRecording);
  });
}
...
PVR_CODEC CPVRClient::cb_get_codec_by_name(const void* kodiInstance, const char* strCodecName)
{
  ...
  HandleAddonCallback(__func__, const_cast<void*>(kodiInstance), [&](CPVRClient* client) {
