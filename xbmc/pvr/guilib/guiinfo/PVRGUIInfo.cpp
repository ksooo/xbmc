...
bool CPVRGUIInfo::GetPVRInt(const CFileItem* item, const CGUIInfo& info, int& iValue) const
{
  ...
  switch (info.m_info)
  {
    ...
    case PVR_ACTUAL_STREAM_SIG_PROGR:
      iValue =
          RoundFloatAndNarrowToInt(static_cast<float>(m_qualityInfo.Signal()) / 0xFFFF * 100.0f);
      return true;
    case PVR_ACTUAL_STREAM_SNR_PROGR:
      iValue = RoundFloatAndNarrowToInt(static_cast<float>(m_qualityInfo.SNR()) / 0xFFFF * 100.0f);
      return true;
    case PVR_BACKEND_DISKSPACE_PROGR:
      if (m_iBackendDiskTotal > 0)
        iValue = RoundFloatAndNarrowToInt(static_cast<float>(m_iBackendDiskUsed) /
                                          m_iBackendDiskTotal * 100.0f);
      else
        iValue = 0xFF;
      return true;
    case PVR_CLIENT_COUNT:
      iValue = UTILS::Narrow<int>(CServiceBroker::GetPVRManager().Clients()->EnabledClientAmount());
      return true;
  }
  return false;
}
...
int CPVRGUIInfo::GetTimeShiftSeekPercent() const
{
  ...
  if (seekSize != 0)
  {
    ...
    const float totalTime = UTILS::Narrow<float>(total);
    if (totalTime == 0.0f)
      return 0;
    ...
    return UTILS::Narrow<int>(std::lrintf(percent));
  }
  return progress;
}
