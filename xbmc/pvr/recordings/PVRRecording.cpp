...
bool CPVRRecording::SetResumePoint(const CBookmark& resumePoint)
{
  ...
  {
    if (client->SetRecordingLastPlayedPosition(*this, std::lrint(resumePoint.timeInSeconds)) !=
        PVR_ERROR_NO_ERROR)
      return false;
  }
                                   ...
                                   const std::string& playerState /* = "" */)
{
  ...
  if (client && client->GetClientCapabilities().SupportsRecordingsLastPlayedPosition())
  {
    if (client->SetRecordingLastPlayedPosition(*this, std::lrint(timeInSeconds)) !=
        PVR_ERROR_NO_ERROR)
      return false;
  }
