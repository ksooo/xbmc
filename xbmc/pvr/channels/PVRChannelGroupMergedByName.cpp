    ...
    const std::vector<std::shared_ptr<CPVRChannelGroup>>& allChannelGroups) const
{
  ...
  for (const auto& member : m_members)
  {
    for (const auto& group : allChannelGroups)
    {
      ...
      {
        if (matchingGroup != -1 && matchingGroup != group->GroupID())
        {
...
{
  std::unordered_map<std::string, unsigned int> knownNames;
  ...
  for (const auto& group : groups)
  {
    ...
    switch (group->GetOrigin())
    {
      case CPVRChannelGroup::Origin::SYSTEM:
      {
        ...
        {
          const auto it = knownNames.find(groupName);
          if (it == knownNames.end())
            knownNames.insert({groupName, 0}); // remember we found a merged group
          else
      ...
      case CPVRChannelGroup::Origin::CLIENT:
      {
        const auto it = knownNames.find(groupName);
        if (it == knownNames.end())
          knownNames.insert({groupName, 1}); // first occurance
        else if ((*it).second > 0)
    ...
    const std::vector<std::shared_ptr<CPVRChannelGroup>>& allChannelGroups)
{
  ...
  {
    const std::string groupName{StringUtils::Format(g_localizeStrings.Get(856), name)};
