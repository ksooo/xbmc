  ...
  m_bFullTextEpgSearch(false)
{
  ...
  if (g_PVRClients->SupportsTimers(m_iClientId))
  {
    ...
    m_timerType = CPVRTimerType::CreateFromAttributes(PVR_TIMER_TYPE_IS_MANUAL, PVR_TIMER_TYPE_IS_REPEATING, m_iClientId);

    if (!m_timerType)
    {
      // last resort. default to first available type from any client.
      m_timerType = CPVRTimerType::GetFirstAvailableType();
    }
  ...
  m_strDirectory(timer.strDirectory)
{
  ...
  m_timerType          = CPVRTimerType::CreateFromIds(timer.iTimerType, m_iClientId);
  ...
  if (!m_timerType)
    CLog::Log(LOGERROR, "%s: no timer type!", __FUNCTION__);
...
CPVRTimerInfoTagPtr CPVRTimerInfoTag::CreateFromEpg(const CEpgInfoTagPtr &tag, bool bRepeating /* = false */)
{
  ...
  if (bRepeating)
  {
    ...
    newTag->SetTimerType(CPVRTimerType::CreateFromAttributes(
