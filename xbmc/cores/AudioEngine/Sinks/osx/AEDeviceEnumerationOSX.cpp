...
AESampleRateList AEDeviceEnumerationOSX::getSampleRateListForStream(UInt32 streamIdx) const
{
  ...
  for(UInt32 formatIdx = 0; formatIdx < formatList.size(); formatIdx++)
  {
    ...
    {
      CLog::Log(LOGINFO, "AE device enumeration: Reported samplerate is kAudioStreamAnyRate; "
                         "adding 44.1khz and 48khz");
...
std::string AEDeviceEnumerationOSX::getExtraDisplayNameForStream(UInt32 streamIdx) const
{
  ...
  if (m_caStreamInfos.size() > 1 && !m_isPlanar)
  {
    ...
    CLog::Log(LOGINFO,
              "AE device enumeration: Adding stream {} as pseudo device with start channel {} and "
              "{} channels total",
              static_cast<unsigned int>(streamIdx), static_cast<unsigned int>(startChannel),
...
void AEDeviceEnumerationOSX::GetAEChannelMap(CAEChannelInfo &channelMap, unsigned int channelsPerFrame) const
{
  ...
  if (logMapping)
  {
    CLog::LogF(LOGDEBUG, "Engine requests layout {}", static_cast<std::string>(channelMap));
  ...
  if (logMapping)
    CLog::LogF(LOGDEBUG, "Mapped channels to layout {}", static_cast<std::string>(channelMap));
}
