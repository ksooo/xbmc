...
public:
  ...
  CPVREpgTagsContainer(int iEpgID,
    ...
    : m_iEpgID(iEpgID), m_channelData(channelData), m_database(database)
  {
  }
   ...
   * @return The tag or nullptr if no tag was found.
   */
  std::shared_ptr<CPVREpgInfoTag> GetNextUpcomingTag() const;

  /*!
   * @brief Get the event that occured previously
   * @return The tag or nullptr if no tag was found.
   */
  std::shared_ptr<CPVREpgInfoTag> GetPreviouslyEndedTag() const;
...
private:
  ...
  mutable std::shared_ptr<CPVREpgInfoTag> m_previouslyEndedTag;
  mutable std::shared_ptr<CPVREpgInfoTag> m_nowActiveTag;
  mutable std::shared_ptr<CPVREpgInfoTag> m_nextUpcomingTag;
