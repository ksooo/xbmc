...
int CPVRGUITimesInfo::GetTimeshiftProgress() const
{
  ...
  return UTILS::Narrow<int>(std::lrintf(current.count() / total.count() * 100));
}
...
int CPVRGUITimesInfo::GetTimeshiftProgressPlayPosition() const
{
  ...
  return UTILS::Narrow<int>(std::lrintf(duration.count() / m_iTimeshiftProgressDuration * 100));
}
...
int CPVRGUITimesInfo::GetEpgEventProgress(const std::shared_ptr<const CPVREpgInfoTag>& epgTag) const
{
  std::unique_lock<CCriticalSection> lock(m_critSection);
  if (epgTag && m_playingEpgTag && *epgTag != *m_playingEpgTag)
    return UTILS::Narrow<int>(std::lrintf(epgTag->ProgressPercentage()));
  else
    return UTILS::Narrow<int>(
        std::lrintf(UTILS::Narrow<float>(GetElapsedTime()) / m_iDuration * 100));
}
