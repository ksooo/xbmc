...
{
  CLog::Log(LOGINFO, "Addon database: create addons table");
...
{
  CLog::Log(LOGINFO, "Addon database: creating indices");
...
bool CAddonDatabase::SetLastUpdated(const std::string& addonId, const CDateTime& dateTime)
{
  ...
  {
    CLog::LogF(LOGERROR, "failed, addon '{}'", addonId);
  }
  return false;
}
                                             ...
                                             const std::vector<AddonInfoPtr>& addons)
{
  try
  {
    ...
    for (const auto& addon : addons)
    {
      ...
      if (idAddon <= 0)
      {
        CLog::LogF(LOGERROR, "insert failed, addon '{}'", addon->ID());
        RollbackTransaction();
        return false;
      }
