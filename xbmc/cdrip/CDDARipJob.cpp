...
{
  CLog::Log(LOGINFO, "CDDA Rip: Start ripping track {} to {}", m_input, m_output);
  ...
  {
    CLog::Log(LOGINFO, "CDDA Rip: Finished ripping {}", m_input);
    if (m_eject)
    {
      CLog::Log(LOGINFO, "CDDA Rip: Ejecting CD");
