...
void CGUIDialogPVRTimerSettings::SetTimer(CFileItem *item)
{
  ...
  m_bAnytime     = m_bIsNewTimer || m_timerInfoTag->IsStartAtAnyTime();
  ...
  m_iDay               = InitializeDay(startLocalTime);
  ...
  m_bNewEpisodesOnly   = m_timerInfoTag->m_bNewEpisodesOnly;
  ...
  /* Select channel entry according to the timer info tag data. */
  ...
  if (m_channel.channelUid == PVR_INVALID_CHANNEL_UID)
  {
    /* Select first channel entry. */
    m_channel = m_channelEntries.begin()->second;
  }
...
void CGUIDialogPVRTimerSettings::InitializeSettings()
{
  ...
  // Timer type
  setting = AddSpinner(group, SETTING_TMR_TYPE, 803, 0, 0, TypesFiller);
  ...
  // Weekdays (only for repeating timers)
  setting = AddButton(group, SETTING_TMR_WEEKDAYS, 19079, 0);
  ...
  setting = AddToggle(group, SETTING_TMR_ANYTIME, 810, 0, m_bAnytime);
  ...
  // Begin day
  setting = AddSpinner(group, SETTING_TMR_DAY, 811, 0, m_iDay, DaysFiller);
  AddTypeDependentVisibilityCondition(setting, SETTING_TMR_DAY);
  AddTypeDependentEnableCondition(setting, SETTING_TMR_DAY);
  AddAnytimeDependentVisibilityCondition(setting, SETTING_TMR_DAY);

  // Begin and end time (hours + minutes only, no day, month, year)
  setting = AddButton(group, SETTING_TMR_BEGIN, 19080, 0);
  ...
  setting = AddButton(group, SETTING_TMR_END, 19081, 0);
  ...
  setting = AddToggle(group, SETTING_TMR_NEW_EPISODES, 812, 0, m_bNewEpisodesOnly);
  ...
  // Priority
  setting = AddSpinner(group, SETTING_TMR_PRIORITY, 19082, 0, m_iPriority, 0, 1, 99);
  ...
  // Lifetime
  setting = AddSpinner(group, SETTING_TMR_LIFETIME, 19083, 0, m_iLifetime, 0, 1, 365, 17999);
...
void CGUIDialogPVRTimerSettings::OnSettingChanged(const CSetting *setting)
{
  ...
  {
    m_bAnytime = dynamic_cast<const CSettingBool*>(setting)->GetValue();
  }
  else if (settingId == SETTING_TMR_DAY)
  {
    m_iDay = dynamic_cast<const CSettingInt*>(setting)->GetValue();
  }
  ...
  {
    m_bNewEpisodesOnly = dynamic_cast<const CSettingBool*>(setting)->GetValue();
  }
...
void CGUIDialogPVRTimerSettings::OnSettingAction(const CSetting *setting)
{
  ...
  {
    /* Load timer weekdays settings dialog */
    CGUIDialogPVRTimerWeekdaysSettings* pDlg
      = dynamic_cast<CGUIDialogPVRTimerWeekdaysSettings*>(
          g_windowManager.GetWindow(WINDOW_DIALOG_PVR_TIMER_WEEKDAYS_SETTING));

    if (pDlg)
    {
      pDlg->SetWeekdays(m_iWeekdays);

      pDlg->DoModal();

      if (pDlg->IsConfirmed())
      {
        m_iWeekdays = pDlg->GetWeekdays();
        SetButtonLabels();
      }
    }
    else
      CLog::Log(LOGERROR, "CGUIDialogPVRTimerSettings::OnSettingAction - Unable to get WINDOW_DIALOG_PVR_TIMER_WEEKDAYS_SETTING instance");
  }
}
...
void CGUIDialogPVRTimerSettings::Save()
{
  ...
  // Begin and end time
  if (m_bAnytime && m_timerType->IsRepeatingEpgBased())
  {
    time_t time = 0;
  ...
  else
  {
    ...
    CDateTime newStart(timestart + CDateTimeSpan(m_iDay - time_diff, 0, 0, -timestart.GetSecond()));
    CDateTime newEnd  (timeend   + CDateTimeSpan(m_iDay - time_diff, 0, 0, -timeend.GetSecond()));
  ...
  // Days (only for repeating timers)
  ...
  // "New episodes only" (only for repeating timers)
  m_timerInfoTag->m_bNewEpisodesOnly = m_bNewEpisodesOnly;
...
void CGUIDialogPVRTimerSettings::InitializeChannelsList()
{
  ...
  /* For epg-based repeating timers, add special "any channel" entry. */
  ...
  const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  for (auto it = pThis->m_channelEntries.begin(); it != pThis->m_channelEntries.end(); ++it)
  {
    ...
    {
      /* For repeating epg-based timers only, add an "any channel". */
