...
std::shared_ptr<CFileItem> LoadVideoFilesFolderInfo(CFileItem& folder)
{
  ...
  loadedItem->SetProperty("totalepisodes", 0);
  loadedItem->SetProperty("watchedepisodes", 0);
  loadedItem->SetProperty("unwatchedepisodes", 0);
  loadedItem->SetProperty("inprogressepisodes", 0);

  for (const auto& item : items)
  {
    if (item->HasVideoInfoTag())
    {
      loadedItem->IncrementProperty("totalepisodes", 1);
      if (item->GetVideoInfoTag()->GetPlayCount() == 0)
      {
        loadedItem->IncrementProperty("unwatchedepisodes", 1);
      }
      else
      {
        loadedItem->IncrementProperty("watchedepisodes", 1);
      }
      if (item->GetVideoInfoTag()->GetResumePoint().IsPartWay())
      {
        loadedItem->IncrementProperty("inprogressepisodes", 1);
      }
    }
  }
  return loadedItem;
