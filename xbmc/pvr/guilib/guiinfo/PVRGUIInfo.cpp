...
bool CPVRGUIInfo::GetPVRInt(const CFileItem* item, const CGUIInfo& info, int& iValue) const
{
  ...
  switch (info.m_info)
  {
    ...
    case PVR_ACTUAL_STREAM_SIG_PROGR:
      iValue = RoundFloatAndNarrowToInt(m_qualityInfo.Signal() / 0xFFFF * 100.0f);
      return true;
    case PVR_ACTUAL_STREAM_SNR_PROGR:
      iValue = RoundFloatAndNarrowToInt(m_qualityInfo.SNR() / 0xFFFF * 100.0f);
      return true;
    case PVR_BACKEND_DISKSPACE_PROGR:
      if (m_iBackendDiskTotal > 0)
        iValue = RoundFloatAndNarrowToInt(m_iBackendDiskUsed / m_iBackendDiskTotal * 100.0f);
      else
        iValue = 0xFF;
      return true;
