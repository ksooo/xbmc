...
public:
  ...
  const std::map<std::string, DigitalButton, std::less<>>& DigitalButtons() const
  {
    return m_digitalButtons;
  }
  const std::map<std::string, AnalogButton, std::less<>>& AnalogButtons() const
  {
    return m_analogButtons;
  }
  const std::map<std::string, AnalogStick, std::less<>>& AnalogSticks() const
  {
    return m_analogSticks;
  }
  const std::map<std::string, Accelerometer, std::less<>>& Accelerometers() const
  {
    return m_accelerometers;
  }
  const std::map<std::string, Throttle, std::less<>>& Throttles() const { return m_throttles; }
  const std::map<std::string, Wheel, std::less<>>& Wheels() const { return m_wheels; }
...
private:
  ...
  // Input state
  std::map<std::string, DigitalButton, std::less<>> m_digitalButtons;
  std::map<std::string, AnalogButton, std::less<>> m_analogButtons;
  std::map<std::string, AnalogStick, std::less<>> m_analogSticks;
  std::map<std::string, Accelerometer, std::less<>> m_accelerometers;
  std::map<std::string, Throttle, std::less<>> m_throttles;
  std::map<std::string, Wheel, std::less<>> m_wheels;
};
