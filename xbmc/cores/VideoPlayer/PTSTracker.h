...
class CPtsTracker
{
  public:
    ...
    size_t GetPatternLength() { return m_patternlength; }
  ...
  private:
    ...
    size_t m_ringfill; //how many diffs we have in the ringbuffer
    double GetDiff(size_t diffnr); //gets diffs from now to the past
    ...
    static bool MatchDifftype(int diffs1[],
                              int diffs2[],
                              size_t nrdiffs); //checks if the difftypes match
    ...
    size_t m_patternlength; //for the codec info
