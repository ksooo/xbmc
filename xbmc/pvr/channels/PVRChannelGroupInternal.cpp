...
std::shared_ptr<CPVRChannel> CPVRChannelGroupInternal::UpdateFromClient(const std::shared_ptr<CPVRChannel> &channel, const CPVRChannelNumber &channelNumber)
{
  CSingleLock lock(m_critSection);
...
bool CPVRChannelGroupInternal::AddToGroup(const std::shared_ptr<CPVRChannel> &channel, const CPVRChannelNumber &channelNumber, bool bUseBackendChannelNumbers)
{
  bool bReturn(false);
  CSingleLock lock(m_critSection);
...
bool CPVRChannelGroupInternal::LoadFromClients(void)
{
  ...
  return CServiceBroker::GetPVRManager().Clients()->GetChannels(this, m_failedClientsForChannels) == PVRClientError::NO_ERROR;
}
...
void CPVRChannelGroupInternal::CreateChannelEpg(const std::shared_ptr<CPVRChannel> &channel, bool bForce /* = false */)
{
  if (channel)
