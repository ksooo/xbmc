...
int SqliteDatabase::connect(bool create)
{
  ...
  try
  {
    ...
    else if (errorCode == SQLITE_OK)
    {
      ...
      if (setErr(sqlite3_exec(getHandle(), "PRAGMA empty_result_callbacks=ON", nullptr, nullptr,
                              nullptr),
...
bool SqliteDatabase::exists()
{
  ...
  const std::string sqlcmd{"SELECT * FROM sqlite_master"};
...
int SqliteDatabase::postconnect()
{
  ...
  if (setErr(sqlite3_exec(getHandle(), cmd.c_str(), nullptr, nullptr, nullptr), cmd.c_str()) !=
      SQLITE_OK)
  {
    throw DbErrors("%s", getErrorMsg());
  }
...
int SqliteDatabase::drop_analytics()
{
  ...
  std::string sqlcmd{"SELECT name FROM sqlite_master WHERE type == 'index' AND sql IS NOT NULL"};
  last_err = sqlite3_exec(conn, sqlcmd.c_str(), &callback, &res, nullptr);
  if (last_err != SQLITE_OK)
  ...
  sqlcmd = "SELECT name FROM sqlite_master WHERE type == 'view'";
  last_err = sqlite3_exec(conn, sqlcmd.c_str(), &callback, &res, nullptr);
  if (last_err != SQLITE_OK)
    return DB_UNEXPECTED_RESULT;
  ...
  sqlcmd = "SELECT name FROM sqlite_master WHERE type == 'trigger'";
  last_err = sqlite3_exec(conn, sqlcmd.c_str(), &callback, &res, nullptr);
  if (last_err != SQLITE_OK)
    return DB_UNEXPECTED_RESULT;
...
void SqliteDataset::fill_fields()
{
  ...
  if (fields_object->size() == 0) // Filling columns name
  {
    ...
    for (size_t i = 0; i < ncols; ++i)
    {
      ...
      name2indexMap.try_emplace(str_toLower(name.data()), i);
    }
  }
...
bool SqliteDataset::query(const std::string& query)
{
  ...
  const size_t fs = qry.find("select");
  const size_t fS = qry.find("SELECT");
  if (!(fs >= 0 || fS >= 0))
