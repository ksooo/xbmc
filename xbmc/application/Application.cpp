...
bool CApplication::CreateGUI()
{
  ...
  {
    CLog::LogF(LOGDEBUG, "trying to init {} windowing system", windowSystem);
    ...
    {
      CLog::LogF(LOGDEBUG, "unable to init {} windowing system", windowSystem);
    ...
    {
      CLog::Log(LOGINFO, "Application: using the {} windowing system", windowSystem);
      break;
    }
  }
  ...
  if (!m_pWinSystem)
  {
    CLog::LogF(LOGFATAL, "unable to init windowing system");
    CServiceBroker::UnregisterWinSystem();
    return false;
  }
...
void CApplication::OnApplicationMessage(ThreadMessage* pMsg)
{
  ...
  case TMSG_EXECUTE_OS:
    ...
    if (audioengine)
    {
      ...
      {
        CLog::LogF(LOGWARNING, "Failed to suspend AudioEngine before launching external program");
      }
    }
  ...
  if (addonMgr.GetDisabledAddons(disabledAddons) && !disabledAddons.empty())
  {
    ...
    for (const auto& addon : disabledAddons)
    {
      if (addonMgr.IsAddonDisabledWithReason(addon->ID(), ADDON::AddonDisabledReason::INCOMPATIBLE))
      {
        ...
        {
          CLog::LogF(LOGDEBUG, "enabling the compatible version of [{}].", addon->ID());
